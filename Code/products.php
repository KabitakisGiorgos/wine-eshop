...
<?php 
  include('session.php');
  include('connect.php');
  $merchants="SELECT c_id FROM winesdb.merchants WHERE c_id=".$_SESSION["login_user"].";";
  $users="SELECT c_id FROM winesdb.users WHERE c_id=".$_SESSION["login_user"].";";
  
  $merch_result=mysqli_query($conn,$merchants);
  $user_result=mysqli_query($conn,$users);
  
  if($merch_result->num_rows==1){
    $_SESSION['isMerchant'] = true;
    // echo 'Here';
  }else if($user_result->num_rows==1){
    $_SESSION['isMerchant'] = false;
  }else{
    echo 'Something went reaaaaallyyyyy bad... :/';
  }
  $wines_q="";
  $price="";

  if($_SESSION['isMerchant']){
    $wines_q = "SELECT p_id,name,photo,wholesale FROM winesdb.wines;";   
  }
  else{
    $wines_q = "SELECT p_id,name,photo,retail FROM winesdb.wines;";        
  }
                  ...
                  <div class="row">
                    <div class="col-sm-4">
                      <div class="cardcontainer" >
                        ...
                        <div class="adddiv">
                            <span class="glyphicon glyphicon-plus add" onclick=addItem("'.$row["p_id"].'","'.$price.'","p_id'.$counter.'")></span>               
                        </div>
                      </div>
                    </div>';  
        }
